...
bool ComplanarFaces(const qh_face_t& A, const qh_face_t& B)
{
	qh_face_t C = B;
	FaceMult(C, -1);
	return EqualFaces(A, B) || EqualFaces(A, C);
}
...
void GetValidFaces(qh_context_t* context, TArray<qh_face_t>& ValidFaces)
{
	for (unsigned int i = 0; i < context->nfaces; ++i) {
		if (context->valid[i])
		{
			...
			if (!PointsAreOnOneLine(v0, v1, v2))
...
/**
Merge coplanar faces (if need)
Reorder (if need) first face points in CW order if it has complanar faces for sort them in CCW order further
If first face is triangle - reorder its points in CCW order
If other faces have not complanar faces (they are triangles) - reorder their points in CCW order (if they aren't)
**/
void OrderAndOrMergeFaces(qh_context_t* context, TArray<qh_face_t>& ValidFaces, TArray<Face>& Faces, bool MergeTriangles)
{
	for (int i = 0; i < ValidFaces.Num(); i++)
	{
		...
		//Add First indices to NewFace
		for (int j = 0; j < 3; j++)
		{
			qh_half_edge_t e = context->edges[ValidFaces[i].edges[j]];
			NewFace.Indices.Add(e.to_vertex);
		}
		FPlane plane = QH_Face_To_FPlane(ValidFaces[i]);

		bool NonComplanarFaceFound = false;
		int NonComplanarIndex = 0;

		for (int j = i + 1; j < ValidFaces.Num(); j++)
		{
			if (ComplanarFaces(ValidFaces[i], ValidFaces[j]))
			{
				if (MergeTriangles)
				{
					for (int k = 0; k < 3; k++)
					{
						qh_half_edge_t e = context->edges[ValidFaces[j].edges[k]];
						NewFace.Indices.AddUnique(e.to_vertex);
					}
					ValidFaces.RemoveAt(j);
					j--;
				}
			}
			else
			{
				if (!NonComplanarFaceFound)
				{
					NonComplanarFaceFound = true;
					NonComplanarIndex = j;
					if (!MergeTriangles)
						break;
				}
		...
		//check orientation of last face 
		//if there are complanar faces in the end - ValidFaces[i] is last face
		if (MergeTriangles && i == ValidFaces.Num() - 1 && !NonComplanarFaceFound)
		{
			NonComplanarFaceFound = true;
			NonComplanarIndex = i - 1;
		}
		//search NonComplanarIndex in backward order
		else if (!MergeTriangles && !NonComplanarFaceFound)
		{
			for (int k = i - 1; k >= 0; k--)
			{
				if (!ComplanarFaces(ValidFaces[i], ValidFaces[k]))
				{
					NonComplanarFaceFound = true;
					NonComplanarIndex = k;
					break;
		...
		//if first face isn't triangle - sort its first 3 points in CW order
		//if any face is triangle - sort points in ccw order
		if (NonComplanarFaceFound && NonComplanarIndex > -1 && (i == 0 || NewFace.Indices.Num() == 3))
		{
			for (int k = 0; k < 3; k++)
			{
				qh_half_edge_t e = context->edges[ValidFaces[NonComplanarIndex].edges[k]];
				if (!NewFace.Indices.Contains(e.to_vertex))
				{
					FVector v = context->vertices[e.to_vertex];
					float Distance = plane.PlaneDot(v);
					//first face is triangle - sort points in CCW order
					if (i == 0 && NewFace.Indices.Num() == 3 && Distance < 0)
					{
						Swap(NewFace.Indices[1], NewFace.Indices[2]);
						break;
					}
					else if (i == 0 && NewFace.Indices.Num() == 3 && Distance > 0)
					{
						break;
					}
					//v in front of first face - reorder NewFace's 2nd and 3rd Indices in CW order for build plane on them further and then sort edge points in CCW order
					else if (i == 0 && NewFace.Indices.Num() > 3 && Distance > 0)
					{
						Swap(NewFace.Indices[1], NewFace.Indices[2]);
						break;
					...
					//face is triangle - sort points in CCW order
...
/**
Move triangles to start part of array
And create for triangles TMap of unfound neighbours 
**/
void ReorderTriangles(TArray<Face>& Faces)
{
	bool NoTrianglesLeft = false;
	for (int i = 0; i < Faces.Num(); i++)
	{
		//if face isn't triangle
		if (Faces[i].Indices.Num() > 3)
		{
			NoTrianglesLeft = true;
			for (int j = i + 1; j < Faces.Num(); j++)
			{
				if (Faces[j].Indices.Num() == 3)
				{
					NoTrianglesLeft = false;
					//add unfound neighbours to triangle face
					if (Faces[j].Neighbours.Num() == 0)
					{
						for (int k = 0; k < Faces[j].Indices.Num(); k++)
							Faces[j].Neighbours.Add(k, -1);
					}
					Swap(Faces[i].Number, Faces[j].Number);
					Swap(Faces[i], Faces[j]);
					break;
				}
			}
		}
		else if (Faces[i].Indices.Num() == 3 && Faces[i].Neighbours.Num() == 0)
		{
			for (int k = 0; k < Faces[i].Indices.Num(); k++)
				Faces[i].Neighbours.Add(k, -1);
		}
		if (NoTrianglesLeft)
			break;
	}
...
void SearchLineEdges(qh_context_t* context, TArray<uint32>& LineIndices, Face& Face, TArray<uint32>& TempExcessIndices)
{
	float MaxSize = 0;
	float Size;
	uint32 MaxIndex;
	for (int i = 1; i < LineIndices.Num(); i++)
	{
		Size = (context->vertices[LineIndices[0]] - context->vertices[LineIndices[i]]).Size();
		if (Size > MaxSize)
		{
			MaxSize = Size;
			MaxIndex = i;
	...
	//Move found index to second position
	if (MaxIndex != 1)
		Swap(LineIndices[1], LineIndices[MaxIndex]);
	//Add excess indices
	for (int i = 2; i < LineIndices.Num(); i++)
	{
		TempExcessIndices.Add(LineIndices[i]);
	}
}

void FindNextEdge(qh_context_t* context, Face& Face, TArray<uint32>& SortedPoints, TArray<uint32>& TempExcessIndices)
{
	int IndicesSize = Face.Indices.Num();
	FVector AB, BC;
	//last Indices of sorted in counter-clockwise order indices
	uint32 a = SortedPoints[SortedPoints.Num() - 2];
	uint32 b = SortedPoints[SortedPoints.Num() - 1];

	AB = context->vertices[b] - context->vertices[a];
	float maxCos = -1;
	float cos;
	...
	for (int i = 0; i < IndicesSize; i++)
	{
		...
		if (c == a || c == b ||
			//if SortedPoints.Num() >= 3 - add  SortedPoints[0] to points for checking
			(SortedPoints.Num() < 3 ? SortedPoints.Contains(c) : (SortedPoints.Contains(c) && c != SortedPoints[0]))
			|| TempExcessIndices.Contains(c))
			continue;

		BC = context->vertices[c] - context->vertices[b];
		cos = CosBetweenVerts(AB, BC);
		if (cos == maxCos)
		{
			LineIndices.Add(c);
		}
	...
	if (LineIndices.Num() <= 2)
	{
		SortedPoints.Add(C);		
	}
	else
	{
		SearchLineEdges(context, LineIndices, Face, TempExcessIndices);
		SortedPoints.Add(LineIndices[1]);
	}
...
/**
Sort Face's vertices in CCW order
Face's first edge points must be found earlier
**/
void SearchFaceEdges(qh_context_t* context, Face& Face, uint32& FirstIndex, uint32& SecondIndex, TMap<uint32, uint8>& ExcessIndices, TArray<uint32>& TempExcessIndices)
{
	TArray<uint32> SortedPoints;
	SortedPoints.Add(FirstIndex);
	SortedPoints.Add(SecondIndex);
	while (SortedPoints[0] != SortedPoints.Last())
	{
		FindNextEdge(context, Face, SortedPoints, TempExcessIndices);
	}
	SortedPoints.RemoveAt(SortedPoints.Num() - 1);

	//Fill ExcessIndices
	if (SortedPoints.Num() < Face.Indices.Num())
	{
		for (int i = 0; i < SortedPoints.Num(); i++)
		{
			Face.Indices.Remove(SortedPoints[i]);
		}
		for (int i = 0; i < Face.Indices.Num(); i++)
		{
			ExcessIndices[Face.Indices[i]] = 0;
		}
	}

	Face.Indices = SortedPoints;
	for (int i = 0; i < Face.Indices.Num(); i++)
	{
...
void SearchFirstFaceEdges(qh_context_t* context, Face& FirstFace, TMap<uint32, uint8>& ExcessIndices)
{
	FPlane Plane(context->vertices[FirstFace.Indices[0]], context->vertices[FirstFace.Indices[1]], context->vertices[FirstFace.Indices[2]]);
	//Search centroid of NewFace
	FVector Centroid(0, 0, 0);
	int IndicesSize = FirstFace.Indices.Num();
	for (int i = 0; i < IndicesSize; i++)
	{
		Centroid += context->vertices[FirstFace.Indices[i]];
	}
	Centroid /= FirstFace.Indices.Num();

	//Search furthest point from centroid
	float MaxDistance = 0;
	FVector TempVec;
	float TempVecSize;
	uint32 MaxIndex = 0;
	for (int i = 0; i < IndicesSize; i++)
	{
		TempVec = context->vertices[FirstFace.Indices[i]] - Centroid;
		TempVecSize = TempVec.Size();
		if (TempVecSize > MaxDistance)
		{
			MaxDistance = TempVecSize;
			MaxIndex = i;
		}
	}
	//Move found index to first position
	if (MaxIndex != 0)
		Swap(FirstFace.Indices[0], FirstFace.Indices[MaxIndex]);

	TempVec = context->vertices[FirstFace.Indices[0]] - Centroid;
	
	//Search second index among points at left from TempVec
	float MaxCos = -1;
	FVector FirstEdge;
	FVector DotProd;
	TArray<uint32> LineIndices;	
	TArray<uint32> TempExcessIndices;
	for (int i = 1; i < IndicesSize; i++)
	{		
		FirstEdge = context->vertices[FirstFace.Indices[i]] - context->vertices[FirstFace.Indices[0]];
		DotProd = TempVec ^ FirstEdge;

		//Points are on one line or pint at right from TempVec
		if (DotProd.IsZero() || Plane.PlaneDot(DotProd) > 0)
			continue;

		float Cos = CosBetweenVerts(TempVec, FirstEdge);
		if (Cos == MaxCos)
		{
			LineIndices.Add(FirstFace.Indices[i]);
		}
		else if (Cos > MaxCos)
		{
			if (LineIndices.Num())
				LineIndices.Empty();
			LineIndices.Add(FirstFace.Indices[0]);
			LineIndices.Add(FirstFace.Indices[i]);
	...
	if (LineIndices.Num() <= 2)
	{
		//Move second found index to second position
		if (MaxIndex != 1)
			Swap(FirstFace.Indices[1], FirstFace.Indices[MaxIndex]);		
	}
	else
	{
		SearchLineEdges(context,LineIndices, FirstFace,TempExcessIndices);
		if (LineIndices[1] != FirstFace.Indices[1])
			Swap(FirstFace.Indices[1], FirstFace.Indices[FirstFace.Indices.Find(LineIndices[1])]);
	}

	SearchFaceEdges(context, FirstFace, FirstFace.Indices[0], FirstFace.Indices[1], ExcessIndices, TempExcessIndices);
}

void SearchIndicesAtOtherFaces(qh_context_t* context, TArray<Face>& Faces, uint32& FirstInd, uint32& SecondInd, uint32& NeighbourNumber, uint32& NeighbourEdgeInd,
	TArray <int>& IgnoreFaceNumbers, TMap<uint32, uint8>& ExcessIndices)
{
	for (int i = 0; i < Faces.Num(); i++)
	{
		if (IgnoreFaceNumbers.Contains(i)||Faces[i].AllNeighboursFound)
			continue;
		//face is triangle
		if (Faces[i].Indices.Num() == 3)
		{
			int j = Faces[i].Indices.Find(FirstInd);
			if (j != INDEX_NONE)
			{
				if (Faces[i].Indices[j==2?0:j+1] == SecondInd)
				{
					NeighbourNumber = i;
					NeighbourEdgeInd = j;
					break;
		...
		{
			//face's edges found
			if (Faces[i].Neighbours.Num() != 0)
			{
				int j = Faces[i].Indices.Find(FirstInd);
				if (j != INDEX_NONE)
				{
					if (Faces[i].Indices[j == Faces[i].Indices.Num()-1 ? 0 : j + 1] == SecondInd)
					{
						NeighbourNumber = i;
						NeighbourEdgeInd = j;
						break;
			...
			else
			{
				...
				if (j != INDEX_NONE)
				{
					int k = Faces[i].Indices.Find(SecondInd);
					if (k != INDEX_NONE)
					{
						NeighbourNumber = i;
						NeighbourEdgeInd = 0;
						TArray<uint32>TempExcessIndices;
						SearchFaceEdges(context, Faces[i], Faces[i].Indices[j], Faces[i].Indices[k], ExcessIndices, TempExcessIndices);
						break;
					}
				}				
			}
...
/**
Recursive Search Neighbours
**/
void SearchNextNeighbour(qh_context_t* context, Face& CurrentFace, TArray<Face>& Faces, TMap<uint32, uint8>& ExcessIndices)
{
	if (CurrentFace.AllNeighboursFound)
		return;
	else
	{
		if (!AllNeighboursFound(CurrentFace))
		{
			TArray<int> IgnoreFaceNumbers;
			IgnoreFaceNumbers.Add(CurrentFace.Number);
			for (int i = 0; i < CurrentFace.Neighbours.Num(); i++)
			{
				if (CurrentFace.Neighbours[i] != -1)
					IgnoreFaceNumbers.Add(CurrentFace.Neighbours[i]);
			}
			for (int i = 0; i < CurrentFace.Neighbours.Num(); i++)
			{
				if (CurrentFace.Neighbours[i] == -1)
				{
					//indices for search neighbour face
					uint32 SecondInd = CurrentFace.Indices[i];
					uint32 FirstInd = i == CurrentFace.Indices.Num() - 1 ? CurrentFace.Indices[0] : CurrentFace.Indices[i + 1];

					//Number of Neighbour face and its edge number
					uint32 NeighbourNumber = 0, NeighbourEdgeInd = 0;

					SearchIndicesAtOtherFaces(context, Faces, FirstInd, SecondInd, NeighbourNumber, NeighbourEdgeInd, IgnoreFaceNumbers, ExcessIndices);

					//Add neighbours to faces
					CurrentFace.Neighbours[i] = NeighbourNumber;
					Faces[NeighbourNumber].Neighbours[NeighbourEdgeInd] = CurrentFace.Number;

					IgnoreFaceNumbers.Add(NeighbourNumber);
				}
			}
		}
		CurrentFace.AllNeighboursFound = true;
		for (int i = 0; i < CurrentFace.Neighbours.Num(); i++)
		{
			SearchNextNeighbour(context, Faces[CurrentFace.Neighbours[i]], Faces, ExcessIndices);
		}
	}
}

void SearchNeighbours(qh_context_t* context, TArray<Face>& Faces, TMap<uint32, uint8>& ExcessIndices, bool MergeTriangles)
{
	//Reorder Faces - move triangles to start part of array
	//And fill their neighbours as unfound 
	ReorderTriangles(Faces);

	//If first Face isn't triangle
	if(Faces[0].Indices.Num()>3)
		SearchFirstFaceEdges(context, Faces[0], ExcessIndices);

	SearchNextNeighbour(context, Faces[0], Faces, ExcessIndices);
}
...
**/
void FillExcessIndices(TMap<uint32, uint8>& ExcessIndices, const uint32& nvertices)
{
	for (uint32 i = 0; i < nvertices; i++)
	{
		ExcessIndices.Add(i, 1);
	}
}

bool HasExcessVerts(TMap<uint32, uint8>& ExcessIndices, const uint32& nvertices)
{
	for (uint32 i = 0; i < nvertices; i++)
	{
...
**/
void VerticesAndIndicesCorrection(TArray<FVector>& Vertices, TArray<Face>& Faces, TMap<uint32, uint8>& ExcessIndices, const uint32& nvertices)
{
...
{	
	...
	TMap<uint32,uint8> ExcessIndices;
	FillExcessIndices(ExcessIndices, nvertices);

	SearchNeighbours(&context, Faces, ExcessIndices, MergeTriangles);
	...
	if (HasExcessVerts(ExcessIndices, nvertices))
	{
		VerticesAndIndicesCorrection(Vertices, Faces, ExcessIndices, nvertices);		
	}
