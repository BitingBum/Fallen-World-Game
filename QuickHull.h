...
bool qh__6eps_only_2_uniques(qh_vertex_t* vertices, unsigned int nvertices, qh_index_t* eps)
{
	...
	if (UniqueIndices.Num() == 2)
	{
		...
		bool FirstIndexHasNoDuplicates = false;
		int SecondDuplicateIndex = -1;
		qh__find_second_duplicate(eps, UniqueIndices[0], SecondDuplicateIndex);
		if (SecondDuplicateIndex == -1)
		{
			FirstIndexHasNoDuplicates = true;
			qh__find_second_duplicate(eps, UniqueIndices[1], SecondDuplicateIndex);
		}
	...
	while (topface != -1) {
		...
		// Sort horizon edges in CCW order
		{
			...
			if (vindex == 3) {
				...
				FPlane Plane(triangle[0], triangle[1], triangle[2]);
				float dist = Plane.PlaneDot(*(context->vertices + apex));
				TryTruncateFloatToZero(dist);
